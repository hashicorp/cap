...
func (p *Provider) Exchange(ctx context.Context, oidcRequest Request, authorizationState string, authorizationCode string) (*Tk, error) {
	...
	log.Printf("TEMPORARY_DEBUG: jwks=%s", jwks)

	// Log the ID token before verification to see its header and signature
	log.Printf("TEMPORARY_DEBUG: id_token=%s", string(t.IDToken()))
